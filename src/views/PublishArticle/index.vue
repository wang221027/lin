...
const publishArticle = async () => {
    ...
    if (localStorage.getItem("nickname")) {
        userStore.nickname = localStorage.getItem("nickname") as string
    }
    const result = await reqPublishArticle(title.value, valueHtml.value, radioData, userStore.nickname, localStorage.getItem("user_id") as string)
    if (result.status === 200) {
        ElMessage.success('发布成功！')
        $router.push('/articleHome')
    }
}
onMounted(() => {
    clearImage()
...
<template>
    <div class="bg">
        <div style="text-align: center;margin-top: 20px;">
            ...
            <Editor style="height: 600px; overflow-y: hidden;width:70%;border: 1px solid #ccc;margin: 0 auto;"
                v-model="valueHtml" :defaultConfig="editorConfig" :mode="mode" @onCreated="handleCreated" />
        </div>
        <!-- 添加封面 -->
        <section class="articleCover">
            <div>
                文章类型：
